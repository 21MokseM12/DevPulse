...
class GithubResponseMapperTest {
    ...
    void mapToIssue_whenBodyLongerThan200_shouldUseFirst200Symbols() {
        ...
        URI repoLink = URI.create("https://github.com/acme/repo");
        GithubResponse response = new GithubResponse(
                ...
                new GithubPayload(
                        "opened",
                        null,
                        new GithubIssue("Issue", issueBody, List.of(), "https://github.com/acme/repo/issues/1")));

        LinkUpdateDTO update = GithubResponseMapper.mapToIssue(response, repoLink);

        assertEquals("a".repeat(200) + "...", update.descriptionPreview());
        assertEquals(URI.create("https://github.com/acme/repo/issues/1"), update.eventUrl());
    }
    ...
    void mapToPullRequest_whenBodyLongerThan200_shouldUseFirst200Symbols() {
        ...
        GithubResponse response = new GithubResponse(
                ...
                new GithubPayload(
                        "opened",
                        new GithubPullRequest("PR", pullRequestBody, List.of(), "https://github.com/acme/repo/pull/2"),
                        null));

        LinkUpdateDTO update = GithubResponseMapper.mapToPullRequest(response, repoLink);

        assertEquals("b".repeat(200) + "...", update.descriptionPreview());
        assertEquals(URI.create("https://github.com/acme/repo/pull/2"), update.eventUrl());
    }
    ...
    void mapToCommit_whenCommitsPresent_shouldBuildCompactSummaryDescription() {
        ...
        LinkUpdateDTO update = GithubResponseMapper.mapToCommit(response, repoLink);

        assertEquals(
                "Push в main: 2 коммита (3f5c1e8), Fix retry logic; Add cache invalidation",
                update.descriptionPreview());
        assertEquals(
                URI.create("https://github.com/acme/repo/compare/19fe5a1c0d2f7731...3f5c1e8e2370a49d"),
                update.eventUrl());
    }
