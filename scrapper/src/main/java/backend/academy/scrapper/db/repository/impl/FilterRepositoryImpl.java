...
public class FilterRepositoryImpl implements FilterRepository {
    ...
    public void save(Set<String> filters, Long linkId) {
        MapSqlParameterSource[] filterSources = filters.stream()
                .map(filter ->
                        new MapSqlParameterSource().addValue(FILTER, filter).addValue(LINK_ID, linkId))
                .toArray(MapSqlParameterSource[]::new);

        jdbcTemplate.batchUpdate(FilterQuery.INSERT_BATCH.query(), filterSources);
    }
    ...
    public Set<String> deleteByLinkId(Long linkId) {
        ...
        jdbcTemplate.query(
                FilterQuery.DELETE_BY_LINK_ID.query(), new MapSqlParameterSource().addValue(LINK_ID, linkId), rs -> {
