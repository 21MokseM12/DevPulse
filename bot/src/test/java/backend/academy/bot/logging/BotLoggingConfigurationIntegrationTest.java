...
class BotLoggingConfigurationIntegrationTest {
    ...
    void prodConfiguration_writesJsonWithTracingFieldsAndMaskedSecrets() throws Exception {
        ...
        try {
            Logger logger = context.getLogger(BotLoggingConfigurationIntegrationTest.class);
            capture.start();
            logger.addAppender(capture);
            MDC.put("traceId", "trace-101");
            MDC.put("spanId", "span-202");
            MDC.put("requestId", "req-303");

            logger.info("token=abc123 request processed");
    ...
    void devConfiguration_staysHumanReadable() throws Exception {
        ...
        capture.start();
        logger.addAppender(capture);
        logger.info("password=qwerty dev logger mode");
        ...
        assertThat(appender).isNotNull();
        assertThat(event.getLevel().levelStr).isEqualTo("INFO");
        assertThat(maskedMessage).contains("password=***");
        assertThat(maskedMessage).contains("dev logger mode");
